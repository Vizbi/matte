...
def get_visualization_and_data(visualization, selected_visualization_variables={}):
    """
    ...
    if visualization.user_database.db_engine == 'bigquery' and visualization.chart_type != 'html':
        ...
        access_token = AccessToken.objects.get(application=app, user=visualization.user)
        ...
        if visualization.measure_infos or visualization.dimension_infos:
            ...
            variables = visualization.visualizationvariable_set.filter(id__in=visualization_variable_ids)
            variables_dict = {}
            for visualization_variable in variables:
                variables_dict[visualization_variable.name] = selected_visualization_variables[str(visualization_variable.id)]
            ...
            database = Database(user_database=visualization.user_database)
            data = database.get_chartable_data_for_query(visualization.raw_query)
...
def compute_chartable_data(visualization, dataframe):
    ...
    pickle_file_path = os.path.join(settings.NOTEBOOK_DIR, visualization.user.username, pickle_filename)
    with open(pickle_file_path, 'w') as f:
        pickle.dump(dataframe, f)

    # Pickle dump the output after running the notebook.
    ipy_fname = os.path.join(settings.NOTEBOOK_DIR, visualization.user.username, notebook_name)
    ...
    output_fname = os.path.join(settings.RECOMPUTED_NOTEBOOK_DIR, visualization.user.username, notebook_name)
    ...
    if settings.DEBUG:
        ...
        os.environ['DOCKER_CERT_PATH'] = '/Users/akshar/.docker/machine/machines/default'
        cli = Client(version='auto', **kwargs_from_env())
    else:
        cli = Client(base_url='unix://var/run/docker.sock')
    environment = {'NOTEBOOK_NAME': notebook_name}
    binds = {}
    binds[output_fname] = {'bind': '/code/{0}'.format(notebook_name,), 'mode': 'rw'}
    binds[pickle_file_path] = {'bind': '/code/{0}'.format(pickle_filename,), 'mode': 'rw'}
    output_dir = os.path.join(settings.NOTEBOOK_OUTPUT_DIR, visualization.user.username)
    ...
    volumes = ['/code/{0}'.format(notebook_name,), '/code/{0}'.format(pickle_filename,), '/outputs']
    resp = cli.create_container('notebook_runner', environment=environment, host_config=cli.create_host_config(binds=binds), volumes=volumes)
    if resp['Id']:
        cli.start(resp['Id'])
